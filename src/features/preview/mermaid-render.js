...
// AI Diagram v2의 다중 시안 카드 미리보기와 PDF Export(headless 브라우저)에서도
// 이 함수를 그대로 재사용할 수 있도록 el.previewBody 같은 특정 DOM에 의존하지 않고
// container를 인자로 받는다.
...
// PDF Export도 문서 Preview와 동일한 렌더링을 목표로 하므로 이 고정 팔레트를 그대로 쓴다.
export const DOCS_BUILDER_DARK_MERMAID_THEME_VARIABLES = {
  primaryColor: "#171c26",
...
export function paletteToThemeVariables(palette) {
...
export const FLOWCHART_LAYOUT_OPTIONS = {
  nodeSpacing: 60,
  rankSpacing: 75,
  diagramPadding: 20,
  useMaxWidth: true,
...
// container(기본값 document) 안의 모든 .mermaid 블록을 순회하며 렌더링한다.
// 블록 단위로 개별 렌더링해서, 문서에 여러 다이어그램이 있을 때
// 하나가 문법 오류여도 나머지는 정상 렌더링되고 실패한 블록에만 에러가 표시되게 한다.
export async function runMermaid(container = document) {
