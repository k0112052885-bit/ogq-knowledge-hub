...
2. 서버가 꺼져 있으면 백그라운드로 `npm run dev`가 실행되고, 서버가 준비되는 대로
   브라우저가 `http://localhost:7777/admin` 을 자동으로 엽니다. 서버는 터미널
   창과 분리된 백그라운드 프로세스이므로, 터미널 창을 닫아도 계속 실행됩니다.
3. 이미 서버가 켜져 있는 상태에서 다시 더블클릭하면, 서버를 새로 켜지 않고
   브라우저 탭만 다시 엽니다(중복 실행 방지).
4. 실행이 끝나면 터미널 창에 "이 창은 닫아도 안전합니다" 안내가 표시됩니다.
   macOS 터미널 환경설정의 프로필별 "셸 종료 시:" 값이 **"터미널 창 닫기"**로
   되어 있으면 이 시점에 창이 자동으로 닫히고, 기본값인 **"창 유지"**로
   되어 있으면 안내대로 `Cmd+W`로 닫으면 됩니다(설정 위치: 터미널 메뉴 >
   설정 > 프로필 > 셸 탭).
5. 작업이 끝나면 관리자 화면(`/admin`) 하단의 **서버 종료** 버튼을 눌러 서버를
   종료합니다.
...
서버 실행 로그는 프로젝트 폴더의 `logs/docs-builder.log` 파일에 기록됩니다
(Git에는 포함되지 않습니다).
...
3. 다음 중 하나로 이미지를 넣습니다.
   ...
   - **드래그 앤 드롭**: PNG/JPEG/WebP 이미지 파일을 편집기 위로 드래그해서 놓기.
     여러 파일을 한 번에 드롭하면 하나씩 순서대로 업로드되어, 드롭한 순서
     그대로 커서 위치에 이어서 삽입됩니다.
4. 업로드가 시작되면 "업로드 중..." 토스트가, 끝나면 "업로드 완료" 토스트가
   표시됩니다(여러 장이면 몇 개 중 몇 개가 삽입됐는지 표시). 완료 즉시 커서
   위치에 `![image](images/파일명)` 형태의 Markdown이 자동 삽입되고, 우측
   미리보기도 곧바로 갱신됩니다. 삽입 후 문서는 "저장 안 됨" 상태가 되며,
   **저장** 버튼(또는 `Cmd/Ctrl+S`)을 눌러야 `docs/*.md`에 실제로 반영됩니다.
5. 이미지 파일은 `docs/images/` 폴더에 **현재 열려있는 문서명 기반**의 안전한
   파일명(예: `07_goal-1.png`, 같은 문서에 또 올리면 `07_goal-2.png`)으로
   저장됩니다. 한글/공백/특수문자는 자동으로 제거되며, slug화 결과가 비어있는
   경우(문서를 열지 않은 특수한 경우 등)에는 `image-YYYYMMDD-HHMMSS.png`
   형식으로 대체됩니다. (폴더가 없으면 자동 생성)
6. 업로드된 이미지는 서버에서 자동으로 최적화됩니다: 가로/세로 중 긴 변이
   1920px를 넘으면 비율을 유지한 채 축소하고, JPEG/WebP는 품질 85%,
   PNG는 최대 압축으로 재인코딩해 용량을 줄입니다(재인코딩 결과가 원본보다
   크면 원본을 그대로 저장). 이미 작은 이미지는 리사이즈되지 않습니다.
7. **빌드**를 실행하면 `docs/images/`가 `dist/images/`로 그대로 복사되어
   실제 사이트에서도 이미지가 표시됩니다.

지원 형식은 PNG, JPEG, WebP이며, 그 외 형식이나 이미지가 아닌 파일을
붙여넣거나 드롭하면 실패 토스트가 표시되고 해당 파일만 건너뜁니다. 여러
파일을 함께 드롭했을 때 일부만 이미지가 아니라면, 이미지가 아닌 파일 개수를
알려주는 토스트가 뜨고 나머지 이미지 파일들은 정상적으로 업로드됩니다.
업로드가 실패한 파일은 Markdown이 삽입되지 않으므로 문서 내용이 바뀌지
않습니다.
...
| POST | `/api/images` | `{ mimeType, data, docSlug? }`(base64 또는 data URL)로 이미지를 `docs/images/`에 저장. PNG/JPEG/WebP만 허용. `docSlug`(문서 파일명에서 `.md` 제거한 값) 기준으로 `{docSlug}-{순번}.ext` 파일명을 생성하고, 없거나 빈 값이면 `image-YYYYMMDD-HHMMSS.png`로 대체. 1920px 초과 이미지는 자동 축소, JPEG/WebP는 품질 85%로 재인코딩. 상대 경로(`{ path: "images/파일명" }`)를 반환 |
